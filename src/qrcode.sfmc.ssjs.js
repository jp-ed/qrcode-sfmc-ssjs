...
QRCodeModel.prototype = {
    ...
    isDark: function(row, col) {
        if (row < 0 || this.moduleCount <= row || col < 0 || this.moduleCount <= col) { Write(row + "," + col); }
    ...
    makeImpl: function(test, maskPattern) {
        ...
        for (var row = 0; row < this.moduleCount; row++) { this.modules[row] = new Array(0); for (var col = 0; col < this.moduleCount; col++) { this.modules[row][col] = undefined; } }
        ...
        if (this.dataCache == undefined) { this.dataCache = QRCodeModel.createData(this.typeNumber, this.errorCorrectLevel, this.dataList); }
...
QRCodeModel.createBytes = function(buffer, rsBlocks) {
    ...
    var maxDcCount = 0;
    var maxEcCount = 0;
    var dcdata = new Array(0);
    var ecdata = new Array(0);
    for (var r = 0; r < rsBlocks.length; r++) {
        var dcCount = rsBlocks[r].dataCount;
        var ecCount = rsBlocks[r].totalCount - dcCount;
        maxDcCount = Math.max(maxDcCount, dcCount);
        maxEcCount = Math.max(maxEcCount, ecCount);
        dcdata[r] = new Array(0);
        for (var i = 0; i < dcCount; i++) { dcdata[r][i] = 0xff & buffer.buffer[i + offset]; }
        offset += dcCount;
        var rsPoly = QRUtil.getErrorCorrectPolynomial(ecCount);
        var rawPoly = new QRPolynomial(dcdata[r], rsPoly.getLength() - 1);
        var modPoly = rawPoly.mod(rsPoly);
        var ecDataRLength = rsPoly.getLength() - 1;
        ecdata[r] = new Array(0);
        for (var i = 0; i < ecDataRLength; i++) {
            var modIndex = i + modPoly.getLength() - ecDataRLength;
...
var QRUtil = {
    ...
    getMask: function(maskPattern, i, j) {
        switch (maskPattern) {
            ...
            default:
                Write("bad maskPattern:" + maskPattern);
        }
    },
...
var QRMath = {
    ...
    LOG_TABLE: [undefined, 0, 1, 25, 2, 50, 26, 198, 3, 223, 51, 238, 27, 104, 199, 75, 4, 100, 224, 14, 52, 141, 239, 129, 28, 193, 105, 248, 200, 8, 76, 113, 5, 138, 101, 47, 225, 36, 15, 33, 53, 147, 142, 218, 240, 18, 130, 69, 29, 181, 194, 125, 106, 39, 249, 185, 201, 154, 9, 120, 77, 228, 114, 166, 6, 191, 139, 98, 102, 221, 48, 253, 226, 152, 37, 179, 16, 145, 34, 136, 54, 208, 148, 206, 143, 150, 219, 189, 241, 210, 19, 92, 131, 56, 70, 64, 30, 66, 182, 163, 195, 72, 126, 110, 107, 58, 40, 84, 250, 133, 186, 61, 202, 94, 155, 159, 10, 21, 121, 43, 78, 212, 229, 172, 115, 243, 167, 87, 7, 112, 192, 247, 140, 128, 99, 13, 103, 74, 222, 237, 49, 197, 254, 24, 227, 165, 153, 119, 38, 184, 180, 124, 17, 68, 146, 217, 35, 32, 137, 46, 55, 63, 209, 91, 149, 188, 207, 205, 144, 135, 151, 178, 220, 252, 190, 97, 242, 86, 211, 171, 20, 42, 93, 158, 132, 60, 57, 83, 71, 109, 65, 162, 31, 45, 67, 216, 183, 123, 164, 118, 196, 23, 73, 236, 127, 12, 111, 246, 108, 161, 59, 82, 41, 157, 85, 170, 251, 96, 134, 177, 187, 204, 62, 90, 203, 89, 95, 176, 156, 169, 160, 81, 11, 245, 22, 235, 122, 117, 44, 215, 79, 174, 213, 233, 230, 231, 173, 232, 116, 214, 244, 234, 168, 80, 88, 175]
};

function QRPolynomial(num, shift) {

    if (num.length == undefined) { Write(num.length + "/" + shift); }
    var offset = 0;
    while (offset < num.length && num[offset] == 0) { offset++; }

    this.numLen = num.length - offset + shift;
...
QRPolynomial.prototype = {
    ...
    getLength: function() { return this.numLen; },
    multiply: function(e) {
        var num = new Array(0);
        for (var i = 0; i < this.getLength(); i++) {
            for (var j = 0; j < e.getLength(); j++) {

                if (num[i + j] == undefined) { num[i + j] = 0; }
...
QRRSBlock.getRSBlocks = function(typeNumber, errorCorrectLevel) {
    ...
    if (rsBlock == undefined) { Write("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectLevel:" + errorCorrectLevel); }
...
QRBitBuffer.prototype = {
    ...
    put: function(pnum, plength) { for (var i = 0; i < plength; i++) { this.putBit(((pnum >>> (plength - i - 1)) & 1) == 1); } },
...
function QRCode(options) {
    ...
    if (typeof this.options.content !== 'string') {
        Write("Expected 'content' as string!");
    }

    if (this.options.content.length === 0) {
        Write("Expected 'content' to be non-empty!");
    }

    if (!(this.options.padding >= 0)) {
        Write("Expected 'padding' value to be non-negative!");
    }

    if (!(this.options.width > 0) || !(this.options.height > 0)) {
        Write("Expected 'width' or 'height' value to be higher than zero!");
    }
    ...
    function _getErrorCorrectLevel(ecl) {
        switch (ecl) {
            ...
            default:
                Write("Unknown error correction level: " + ecl);
        }
    }
    ...
    function _getTypeNumber(content, ecl) {
        ...
        for (var i = 0; i <= QRCodeLimitLength.length; i++) {
            ...
            if (!table) {
                Write("Content too long: expected " + limit + " but got " + length);
            }
...
QRCode.prototype.table = function() {
    var options = this.options || {};
    var modules = this.qrcode.modules;
    var width = options.width;
    var height = options.height;     
    
    //Apply new lines and indents in table
...
QRCode.prototype.svg = function(opt) {
    ...
    var xmlDeclaration = typeof options.xmlDeclaration != "undefined" ? !!options.xmlDeclaration : false;
